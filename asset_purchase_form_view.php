...
<li><a href="view_aps_transmittal.php"><em class="fa fa-envelope-open-o"></em>View Transmittal Form</a></li>
    ...
    <h5 class="mt-2">List of Saved Bill Forms</h5><a class="anchor" name="tables"></a>
    <form method="post" action="asset_purchase_form_view_sr.php">
          <input type="text" placeholder="Search By Amount/Payment For" name="search_query">
        </form>
                ...
                function number_to_word( $num = '' )
{
    ...
    if( ( int ) ( $num ) && ctype_digit( $num ) )
    {
        ...
        foreach( $num_levels as $num_part )
        {
            ...
            $hundreds   = ( $hundreds ? ' ' . $list1[$hundreds] . ' Hundred' . ( $hundreds == 1 ? '' : ' Pesos' ) . ' ' : '' );
...
function str_replace_last( $search , $replace , $str ) {
    ...
    return $str;
              ...
              $status="Approved";
              $query=mysqli_query($con, "SELECT * from asset_purchase_form where status='$status'");
               $query1=mysqli_query($con, "SELECT sum(payment) as total_payment from asset_purchase_form where status='$status'");
              ?>
              <table>
                <thead>
                  <tr>
                ...
                while ($row=mysqli_fetch_array($query)) {
                  ?>
                    ...
                    <td><?php echo $row['payment_purpose']; ?></td>
                     ...
                     <td><?php echo $row['name_of_thing']; ?></td>
                        <td><?php echo $row['quantity']; ?></td>
                           <td><?php echo $row['unit_price']; ?></td>
                              <td><?php echo $row['purchased_from']; ?></td>
                    ...
                    (
                    <?php
                     echo number_to_word( $row['payment']);
                    ?>
                  )
                      </td>
                      <td><?php echo $row['years_to_pay']; ?></td>
                  ...
                  while ($row=mysqli_fetch_array($query1)) {
                    echo "Total Amount: P" . $row['total_payment'] . ".00";
                  }
                   ?>
