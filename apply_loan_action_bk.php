...
if (mysqli_num_rows($select)>0) {
	...
	<script type="text/javascript">
		...
		self.location.replace("my_loans_bk.php");
	</script>
	<?php
}else{
	$loan_amount=$_POST['loan_amount'];
$query=mysqli_query($con, "INSERT into loan_tb (employee_number, loan_amount, years_to_pay,status,date_applied, amortization) values('$employee_number', '$loan_amount', '$years_to_pay', '$status', '$date_today', '$interest')");
